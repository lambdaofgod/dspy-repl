...
def _ensure_sibling_dspy_package() -> None:
    ...
    This keeps benchmark code in dspy-repl while reusing dataset/reporting helpers from
    the sibling dspy project.
    """
...
def _load_oolong_tasks(config: BenchmarkConfig) -> list[dict[str, Any]]:
    _ensure_sibling_dspy_package()
    from benchmarks.datasets.oolong import load_oolong_trec_tasks  # type: ignore[import-not-found]

    tasks = load_oolong_trec_tasks(
    ...
    if not filtered:
        raise ValueError(f"sample_id '{config.dataset.sample_id}' was not found in loaded Oolong samples.")
    return filtered
...
def _oolong_score(predicted: str, expected: str) -> float:
    _ensure_sibling_dspy_package()
    from benchmarks.datasets.oolong import oolong_score_fn  # type: ignore[import-not-found]

    return float(oolong_score_fn(predicted, expected))
    ...
    *,
    ...
    try:
        ...
        if expected is not None:
            score = _oolong_score(str(answer), str(expected))
...
def _run_task_worker(*, language: Language, task: dict[str, Any], config: BenchmarkConfig) -> TaskResult:
    _configure_lm(config)
    return execute_task(language=language, task=task, config=config)
...
def _run_task_across_languages(
    *,
    ...
    if not config.parallel.enabled:
        return [execute_task(language=language, task=task, config=config) for language in languages]
...
def run_oolong_benchmark(config: BenchmarkConfig) -> Path:
    ...
    log_event(logger, "benchmark_start", run_id=run_id, config=config.to_dict())
    _configure_lm(config)
    languages = resolve_runnable_languages(config.run.languages, logger)
    tasks = [_sanitize_task_for_worker(task) for task in _load_oolong_tasks(config)]
    log_event(logger, "dataset_loaded", dataset="oolong_trec", sample_count=len(tasks), languages=languages)
