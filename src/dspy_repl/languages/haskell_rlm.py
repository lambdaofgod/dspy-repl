...
from dspy_repl.interpreters import load_haskell_interpreter

if TYPE_CHECKING:
    from dspy_repl.compat import Signature


class HaskellREPLEntry(REPLEntry):
...
class HaskellREPLHistory(REPLHistory):
    entries: list[HaskellREPLEntry] = Field(default_factory=list)
    ...
    def append(self, *, reasoning: str = "", code: str, output: str) -> HaskellREPLHistory:
        new_entry = HaskellREPLEntry(reasoning=reasoning, code=code, output=output)
        return HaskellREPLHistory(entries=list(self.entries) + [new_entry], max_output_chars=self.max_output_chars)


HASKELL_ACTION_INSTRUCTIONS_TEMPLATE = """You are tasked with producing the following outputs given the inputs {inputs}:
{output_fields}

You have access to a Haskell (GHCi) REPL environment. Write Haskell code and it will be evaluated. You will see the output, then write more code based on what you learned. This is an iterative process.

Available:
- Variables: {inputs} (your input data, accessible by name)
- llmQuery "prompt" - query a sub-LLM for semantic analysis
- llmQueryBatched ["prompt1", "prompt2", ...] - query multiple prompts
- putStrLn value - ALWAYS print to see results
- submit({final_output_names}) - submit final output when done

IMPORTANT: This is ITERATIVE.
1. EXPLORE FIRST - inspect samples, types, and structure.
...
class HaskellRLM(BaseReplRLM):
    _RESERVED_TOOL_NAMES = frozenset({"llmQuery", "llmQueryBatched", "submit", "print", "putStrLn"})
    _CODE_FENCE_PATTERN = re.compile(r"^```(?:haskell|hs)?\s*\n(.*)\n```\s*$", re.DOTALL)
    ...
    def _new_history(self) -> HaskellREPLHistory:
        return HaskellREPLHistory(max_output_chars=self.max_output_chars)

    def _format_output(self, output: str) -> str:
        return output or "(no output - did you forget to print?)"
    ...
    def _create_interpreter(self, execution_tools: dict[str, Callable]) -> CodeInterpreter:
        interpreter_cls = load_haskell_interpreter()
    ...
    def _make_llm_tools(self, max_workers: int = 8) -> dict[str, Callable]:
            ...
            with lock:
                if state["call_count"] + n > self.max_llm_calls:
                    raise RuntimeError(
                        ...
                        "Use Haskell code for aggregation instead of making more LLM calls."
                    )
                state["call_count"] += n
        ...
        def _query_lm(prompt: str) -> str:
            ...
            if target_lm is None:
                raise RuntimeError("No LM configured. Use dspy.configure(lm=...) or pass sub_lm to HaskellRLM.")
    ...
    def _build_signatures(self) -> tuple[Signature, Signature]:
        ...
        action_sig = (
            dspy.Signature(
                {},
                task_instructions
                + HASKELL_ACTION_INSTRUCTIONS_TEMPLATE.format(
                    inputs=inputs_str,
            ...
            .append(
                "code", dspy.OutputField(desc="Haskell code to execute. Use markdown code block format."), type_=str
            )
        )
...
Review your trajectory to see what information you gathered and what values you computed, then provide the final outputs."""
        ...
        extract_sig = extract_sig.prepend(
            "repl_history", dspy.InputField(desc="Your REPL interactions so far"), type_=HaskellREPLHistory
        )
        extract_sig = extract_sig.prepend(
            "variables_info",
