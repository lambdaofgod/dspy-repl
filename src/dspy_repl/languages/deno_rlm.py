...
class DenoREPLHistory(REPLHistory):
    entries: list[DenoREPLEntry] = Field(default_factory=list)

    def format(self) -> str:
        if not self.entries:
            return "You have not interacted with the Deno REPL environment yet."
        return "\n".join(
            entry.format(index=i, max_output_chars=self.max_output_chars) for i, entry in enumerate(self.entries)
        )

    def append(self, *, reasoning: str = "", code: str, output: str) -> DenoREPLHistory:
        new_entry = DenoREPLEntry(reasoning=reasoning, code=code, output=output)
        return DenoREPLHistory(entries=list(self.entries) + [new_entry], max_output_chars=self.max_output_chars)


DENO_ACTION_INSTRUCTIONS_TEMPLATE = """You are tasked with producing the following outputs given the inputs {inputs}:
{output_fields}

You have access to a TypeScript (Deno) REPL environment. Write TypeScript code and it will be evaluated. You will see the output, then write more code based on what you learned. This is an iterative process.
...
- Deno APIs (fetch, file I/O, etc.) and URL imports (e.g. import from "https://...")
...
class DenoRLM(BaseReplRLM):
    _RESERVED_TOOL_NAMES = frozenset({"llmQuery", "llmQueryBatched", "submit", "SUBMIT", "console"})
    _CODE_FENCE_PATTERN = re.compile(r"^```(?:typescript|ts|javascript|js)?\s*\n(.*)\n```\s*$", re.DOTALL)
    ...
    def _create_interpreter(self, execution_tools: dict[str, Callable]) -> CodeInterpreter:
        return DenoInterpreter(tools=execution_tools, output_fields=self._get_output_fields_info())
    ...
    def _make_llm_tools(self, max_workers: int = 8) -> dict[str, Callable]:
            ...
            with lock:
                if state["call_count"] + n > self.max_llm_calls:
                    raise RuntimeError(
                        ...
                        "Use TypeScript for aggregation instead of making more LLM calls."
                    )
                state["call_count"] += n
        ...
        def _query_lm(prompt: str) -> str:
            ...
            if target_lm is None:
                raise RuntimeError("No LM configured. Use dspy.configure(lm=...) or pass sub_lm to DenoRLM.")
    ...
    def _build_signatures(self) -> tuple[Signature, Signature]:
        ...
        action_sig = (
            dspy.Signature(
                {},
                task_instructions
                + DENO_ACTION_INSTRUCTIONS_TEMPLATE.format(
                    inputs=inputs_str,
            ...
            .append(
                "code", dspy.OutputField(desc="TypeScript code to execute. Use markdown code block format."), type_=str
            )
        )
...
Review your trajectory to see what information you gathered and what values you computed, then provide the final outputs."""
        ...
        extract_sig = extract_sig.prepend(
            "repl_history", dspy.InputField(desc="Your REPL interactions so far"), type_=DenoREPLHistory
        )
        extract_sig = extract_sig.prepend(
            "variables_info",
